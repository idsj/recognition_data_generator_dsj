...
class dataAugmentation(object):
    ...
    @classmethod
    def add_erode(cls,img):#适当腐蚀
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT,(3, 3))    
        img = cv2.erode(img,kernel) 
        return img

    @classmethod
    def add_dilate(cls,img):#适当膨胀
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT,(3, 3))    
        img = cv2.dilate(img,kernel) 
        return img

    def do(self,img_list=[]):#随机扰动，默认：50%概率添加噪声；50%的概率膨胀，如果没有膨胀，则腐蚀
...
class PreprocessResizeKeepRatioFillBG(object):
    ...
    def do(self, cv2_img):
        ...
        if len(cv2_img.shape) > 2:
            ...
            print("cv2_img.shape",cv2_img.shape)
        else:
            pix_dim = None
...
def get_label_dict(label_dic_path):
    f=open(label_dic_path,'rb')
    label_dict = pickle.load(f)
    f.close()
    return label_dict

@click.command()
@click.option("--out_dir", default="./dataset", help="write a caffe dir")
@click.option("--font_dir", default="./chinese_fonts", help="font dir to to produce images")
@click.option("--label_dic_path",default="./chinese_labels", help="")
@click.option("--test_ratio",default=0.2, help="test dataset size")
@click.option("--width",default=30, help="width")
@click.option("--height",default=30, help="height")
@click.option("--need_crop",default=False, type=bool,help="cut into certain shape")
@click.option("--margin",default=4, help="")
@click.option("--rotate",default=30, help="max rotate degree 0-45")
@click.option("--rotate_step",default=1, help="rotate step for the rotate angle")
@click.option("--need_aug",default=False, type=bool,help="need data augmentation")
@click.option("--delete_original_data_path",default=False, type=bool, help="Delete the original data path")

#python gen_printed_char.py --out_dir ./dataset --font_dir ./chinese_fonts --width 30 --height 30 --margin 4 --rotate 30 --rotate_step 1
def main(out_dir,font_dir,label_dic_path,test_ratio,width,height,need_crop,margin,rotate,rotate_step,need_aug,delete_original_data_path):

    print("out_dir: %s\nfont_dir: %s\nlabel_dic_path: %s\ntest_ratio: %.2f\nwidth: %d\nheight: %d\nneed_crop: %s\nmargin: %d\nrotate: %d\nneed_aug: %s\nrotate_step: %s\ndelete_original_data_path: %s" %
          (out_dir, font_dir,label_dic_path, test_ratio, width, height,need_crop,margin,rotate,need_aug,rotate_step,delete_original_data_path))
    ...
    print(train_images_dir,test_images_dir)
    #不删除原来的数据
    if delete_original_data_path:
        print("警告，删除原来的数据目录！！")
    ...
    label_dict = get_label_dict(label_dic_path)
    print("label_dict size: %d" % len(label_dict))
    ...
    if rotate > 0 and rotate <= 45:
        ...
        for i in range(-rotate, 0, rotate_step):
            all_rotate_angles.append(i)
    print("旋转角度个数：%d" % len(all_rotate_angles))
    ...
    print("字体个数: %d" % len(verified_font_paths))
    ...
    for (char, value) in lang_chars.items():  # 外层循环是字
        image_list = []
        time = time+1
        print ('{}，{}，进度：{:.2f}'.format(char,value,time/total))
